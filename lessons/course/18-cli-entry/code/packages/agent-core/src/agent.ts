...
import { runAgentLoop, runAgentLoopContinue } from "./agent-loop-full.js";
...
export class Agent {
  ...
  // -----------------------------------------------------------------------
  // Legacy API: startRun / finishRun / pushEvent (for L04 test compat)
  // -----------------------------------------------------------------------

  /**
   * Start a managed run. Sets up the AbortController and activeRun
   * tracking. Used by L04 tests to manually push events.
   */
  ...
  /**
   * End the current run. Used by L04 tests.
   */
  finishRun(): void {
    this._finishRun();
  }
   ...
   * Used by L04 tests to manually push events.
   */
  ...
  private createLoopConfig(options: { skipInitialSteeringPoll?: boolean } = {}): AgentFullLoopConfig {
   ...
   * 4. Guaranteed cleanup via _finishRun()
   */
