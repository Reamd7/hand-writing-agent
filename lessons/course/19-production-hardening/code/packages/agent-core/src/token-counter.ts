 ...
 */
export function estimateTokensChars4(message: ModelMessage): number {
  let chars = 0;
      ...
      if (typeof message.content === "string") {
        ...
        for (const part of message.content) {
          if (part.type === "text") {
            ...
            chars += JSON.stringify((part as any).input).length;
          }
        }
      }
      break;
        ...
        for (const part of message.content) {
          if (part.type === "tool-result") {
            ...
            if (typeof partOutput === "string") {
              chars += partOutput.length;
            } else {
              chars += JSON.stringify(partOutput).length;
            }
          }
        }
      }
