...
async function* mapFullStreamToAgentEvents(
  ...
  for await (const part of fullStream) {
    switch (part.type) {
      ...
      // --- Text streaming ---
      case "text-start": {
        const evt: AssistantStreamEvent = {
          type: "text_start",
          contentIndex,
          partial: { ...partial },
        };
        yield { type: "message_update", message: { ...partial }, streamEvent: evt };
        break;
      }
    ...
    switch (event.type) {
      ...
      case "message_update": {
        ...
        if (streamType === "text_delta") {
          process.stdout.write((event.streamEvent as any).delta);
        } else {
