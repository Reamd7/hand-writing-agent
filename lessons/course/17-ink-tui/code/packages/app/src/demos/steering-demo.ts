...
import { Agent, PendingMessageQueue } from "@my-agent/core";
import type { AgentMessage, AssistantMessage, Usage } from "@my-agent/core";
...
async function demoSteering(): Promise<void> {
  ...
  agent.subscribe((event) => {
    if (event.type === "message_end" && "role" in event.message) {
      ...
      if (msg.role === "user") {
        const uc = typeof msg.content === "string" ? [{ type: "text" as const, text: msg.content }] : msg.content;
        const text = uc.find((c: { type: string }) => c.type === "text");
